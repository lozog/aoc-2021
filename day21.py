...
while True:
    ...
    if player == 0:
        num_remaining_opponents = sum([num_at_score for position in all_p1_scores for num_at_score in position])
    else:
        num_remaining_opponents = sum([num_at_score for position in all_p0_scores for num_at_score in position])
    # print(f"{player} {num_remaining_opponents}")
            ...
            if num_at_score > 0:
                ...
                for roll, roll_count in rolls.items():
                    ...
                    if new_score >= END_SCORE:
                        ...
                        # print(f"player {player} has won in {roll_count * num_at_score * num_remaining_opponents} more universes")
                        num_winning_universes[player] += roll_count * num_at_score * num_remaining_opponents
                    else:
